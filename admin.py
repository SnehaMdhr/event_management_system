...
class AdminRegister:
    def __init__(self, root):
        self.root = root
        self.root.title("Administration Registration Window")
        ...
        title = Label(frame1, text="ADMINISTRATION REGISTRATION", font=("times new roman", 20,), bg="#151E4C", fg="Green")
        ...
        role_label = Label(frame1, text="Role", font=("times new roman", 15), bg="#151E4C", fg="Gray")
        role_label.place(x=50, y=440)
        self.role_entry = Entry(frame1, font=("times new roman", 15), bg="#AE9898")
        self.role_entry.place(x=50, y=470, width=250)

        sign_in = Button(frame1, width=15, text='Sign In', border=0, bg='#407AD1', cursor='hand2', fg='black',
                         font=("times new roman", 15), command=self.validate_fields)
        sign_in.place(x=370, y=460)
        # self.root.destroy()
    ...
    def add(self):
        conn= sqlite3.connect("admin.db")
        c=conn.cursor()
        c.execute("INSERT INTO data(fname,lname,me,pass,day,month,year,gender,role) VALUES (?,?,?,?,?,?,?,?,?)",(self.f_name_entry.get(),self.l_name_entry.get(),self.contact_entry.get(),self.password_entry.get(),self.Day.get(),self.Months.get(),self.Year.get(),self.selected_gender.get(),self.role_entry.get()))
        conn.commit()
        conn.close()
